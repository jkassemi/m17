...
This repository also ships a pure Rust port of the dip test built from the same numerical algorithm. The crate lives at the repository root and can be exercised with `cargo test`. Basic usage mirrors the Python API:

```rust
use diptest::{dipstat, diptest};

let samples = vec![/* sorted or unsorted observations */];
let statistic = dipstat(&samples).expect("valid input");
let result = diptest(&samples).expect("valid input");
println!("dip = {} p_value = {}", result.dip, result.p_value);
```

For more control you can call `dipstat_with_options` or `diptest_with_options` to configure sorting, the `allow_zero` behaviour, full diagnostic output, or bootstrap-based p-values.
Bootstrap p-values can be parallelised via Rayon by setting `BootstrapConfig { threads: Some(n), .. }`.
